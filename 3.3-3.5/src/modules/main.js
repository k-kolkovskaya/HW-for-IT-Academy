...
function addButtonClicked() {
  ...
  //!checkIfColorCanBeAdded(htmlElements.input.value) ? alert("There's no such a color") : checkIfColorAdded(htmlElements.input.value);

  if (htmlElements.input.value === "") {
    alert("Please enter a value first");
  } else if (!checkIfColorCanBeAdded(htmlElements.input.value)) {
...
function chooseColor(color) {
  ...
  // 1.1. Как только i-тое значение одного из option совпало со значением цвета,
  // указанного в качестве входного параметра, вызывает функцию setSelectedIndex, передавая в нее индекс i итератора цикла.
  for (let i = 1; i < htmlElements.selectColor.length; i++) {
    if (htmlElements.selectColor[i].value === color) {
      setSelectedIndex(i);
    }
